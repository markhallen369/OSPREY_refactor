...
public class RCPairVDWChecker {
    ...
    private void initDOFIntervals(RC rc1, RC rc2){
        BBFreeBlockSplitter bfbs = new BBFreeBlockSplitter();
        bfbs.makeDOFMapping();//create sub-block and its DOF
        ...
        if(rc2!=null){
            for(int dof=0; dof<rc2.DOFs.size(); dof++){
                ...
                DegreeOfFreedom curDOF = bfbs.mapDOF( rc2.DOFs.get(dof) );
            ...
            addRCLinConstr(bfbs, rc1);//can do this now because dofIntervals is ready!
            
            if( ! rc2.AAType.equalsIgnoreCase(res2.template.name) )
                res2MutDOF.mutateTo(rc2.AAType);
        }
        
        addRCLinConstr(bfbs, rc1);
    }
    
    
    private void addRCLinConstr(BBFreeBlockSplitter bfbs, RC rc){
        //add rc's non-box constraint to voxLinConstr
        //assumes dofIntervals is ready!
        int numDOFsHere = dofIntervals.size();
        for(LinearConstraint lc : rc.linConstr){
            //convert lc to operate on the space of DOFs in dofIntervals
            double[] coeffs = new double[numDOFsHere];
            for(int dofCount=0; dofCount<rc.DOFs.size(); dofCount++){
                DegreeOfFreedom curDOF = bfbs.mapDOF( rc.DOFs.get(dofCount) );
                
                boolean foundDOFHere = false;
                for(int iDofCount=0; iDofCount<dofIntervals.size(); iDofCount++){
                    if(curDOF==dofIntervals.get(iDofCount).dof){
                        coeffs[iDofCount] = lc.getCoefficients().getEntry(dofCount);
                        foundDOFHere = true;
                        break;
                    }
                }
                ...
                if(!foundDOFHere)
                    throw new RuntimeException("ERROR: Couldn't find DOF in dofIntervals");
            }
            
            LinearConstraint constrHere = new LinearConstraint( coeffs, lc.getRelationship(), lc.getValue() );
            voxLinConstr.add(constrHere);
        }
    }
