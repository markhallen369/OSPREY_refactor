...
public class KStarBeak {
    ...
    public void makeUnprunedClusterExpansion(){
        ...
        for(int pos=0; pos<numPos; pos++){
            ...
            TupleEnumerator tupEnum = new TupleEnumerator(integPruneMat, estIntegEMat, null, numPos);
    ...
    public void testVsExhaustiveLUTE(boolean testEPICToo){
        ...
        for(int t=0; t<numTests; t++){
            ...
            System.out.println("LUTE FREE ENERGY: "+(-IntegrableDOF.RT*Math.log(luteQ)));
            
            if(testEPICToo){
                conf = new int[numPos];
                double epicQ = computeExhaustivePartitionFunction(seq, conf, 0, false);
                System.out.println("EPIC FREE ENERGY: "+(-IntegrableDOF.RT*Math.log(epicQ)));
            }
        }
    }
    ...
    public double computeExhaustivePartitionFunction(int seq[], int conf[], int posOffset,
        ...
        if(posOffset==numPos){
            ...
            if(lutePruningMatrix.isPruned(new RCTuple(conf)))
                ...
                return Math.exp(-E/IntegrableDOF.RT);
            }
        }
        else {
            double ans = 0;
