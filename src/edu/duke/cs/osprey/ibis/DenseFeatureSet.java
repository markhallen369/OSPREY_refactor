...
public interface DenseFeatureSet extends Serializable {
    ...
    class EPIC implements DenseFeatureSet {
        ...
        public int monomialIndexByDOFs(int dof1, int dof2){
            //index of a quadratic term, given dofIndices for the two DOFs
            int quadOffset = dofIndices.length+1;
            int index1 = indexOf(dofIndices, dof1);
            int index2 = indexOf(dofIndices, dof2);
            int upperIndex = Math.max(index1,index2);
            int lowerIndex = Math.min(index1,index2);
            
            return dofIndices.length+1 //for constant and linear terms
                    +(1+upperIndex)*upperIndex/2 //1+...+upperIndex
                    +lowerIndex;
        }
    ...
    static int indexOf(int[] a, int b){
        ...
        throw new RuntimeException("ERROR: Element doesn't appear in array");
    }
