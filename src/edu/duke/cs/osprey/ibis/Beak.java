...
public class Beak implements Serializable {
    ...
    double squareError = 0;//every time we integrate we add error
    //assuming they're independent, it adds up in quadrature
    ...
    public SparseLinearEnergy doIntegration(SparseLinearEnergy input, ArrayList<IntegrableDOF> dofsToIntegrate){
        trainingSamples = generateSamples(input);
        CVSamples = generateSamples(input);
        
        trainingSamples.checkPLUGViolations(plugMat);//DEBUG!!!
        
        SparseLinearEnergy curE = input;
        squareError = 0;
        ...
        System.out.println("TOTAL RMS ERROR FROM INTEGRATION: "+Math.sqrt(squareError));
        
        return curE;
    }
    
    public SparseLinearEnergy refitModel(SparseLinearEnergy input, FeatureSet featSet){
        ...
        ConfSampleSet trainingEnergies = relevantTrainingSamples.evalEnergies(input);
        ...
        ConfSampleSet CVEnergies = relevantCVSamples.evalEnergies(input);
        System.out.print("VALIDATION ");
        double rmsErr = CVEnergies.checkError(newF);
        squareError += rmsErr*rmsErr;
        
        
        return newF;
    }
    
    public SparseLinearEnergy integrateWrtDOF(SparseLinearEnergy input, IntegrableDOF dof){
        ConfSampleSet relevantTrainingSamples = selectSamples(trainingSamples,input);
        ConfSampleSet trainingEnergies = relevantTrainingSamples.integrateEnergies(dof, input);
        SparseLinearEnergy newF = input.integrateModel(dof, trainingEnergies);
        ...
        //DEBUG!!!  might find a more useful way to validate
        ConfSampleSet relevantCVSamples = selectSamples(CVSamples,input);
        ConfSampleSet CVEnergies = relevantCVSamples.integrateEnergies(dof, input);
        System.out.print("VALIDATION ");
        double rmsErr = CVEnergies.checkError(newF);
        squareError += rmsErr*rmsErr;
    ...
    private ConfSampleSet selectSamples(ConfSampleSet superset, SparseLinearEnergy f){
        //DEBUG!!
        //can ultimately just use the needed number of samples per term, but for now keep it simple
        return superset;
    }
    
    private ConfSampleSet generateSamples(SparseLinearEnergy input){
        ...
        if(input instanceof SparseLinearEnergy)
            ...
            ans = new ConfSampleSet( ((SparseLinearEnergy)input).featSet, plugMat, pruneMat );
            //DEBUG!!
        else
    ...
    public static void main(String args[]){
        ...
        /*for(double L1 : new double[] {1e-6,1e-4,1e-3,0.01}){
            DiscreteParamsFittingPartition.regularizationLambda = L1;
            for(double L2 : new double[] {0.0001,0.001,0.01}){
                RCTupleFittingPartition.regularizationLambda = L2;
                System.out.println();
                System.out.println("TRYING OUT FITTING.  DISCR LAMBDA: "+L1+" RCTUP LAMBDA: "+L2);
                System.out.println();
                fullTest();
            }
        }*/
    ...
    public static void fullTest(){
        ...
        SparseLinearEnergy luteModel = curvedBeak.doIntegration(initSLE, dofsToIntegrate);
        ...
        /*
        SparseLinearEnergy luteModel = (SparseLinearEnergy) ObjectIO.readObject("LUTE_MODEL.dat", true);
    ...
    public static void luteKStarTest(){
        ...
        SparseLinearEnergy integModel = curvedBeak.doIntegration(initSLE, dofsToIntegrate);
