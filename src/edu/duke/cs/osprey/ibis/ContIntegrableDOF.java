...
public class ContIntegrableDOF implements IntegrableDOF {
    ...
    //DEBUG!!!!
    final static boolean usePLUGIntegral = false;
    ...
    public double boltzmannIntegratedEnergy(SparseLinearEnergy baseE, ConfSample samp) {
        ...
        switch(dpb.status){
            ...
            case CLASHING:
                ...
                //else fall through, will use full bounds
            case BOUNDED:
                ...
                if(Double.isInfinite(ans) && ContIntegrableDOF.usePLUGIntegral){//DEBUG!!  non-PLUG samples could legitimately be arbitrarily big, overflow and showing inf is ok then
    ...
    double[] calcFullInterval(RCTuple vox){
        return calcFullInterval(vox, plugMtx, dof.getName(), true);
    }
    
    static double[] calcFullInterval(RCTuple vox, PolytopeMatrix plugMat, String dofName, boolean errorIfUndefined){
        for(int posCount=0; posCount<vox.size(); posCount++){
            RC rc = plugMat.cSpace.posFlex.get(vox.pos.get(posCount)).RCs.get(vox.RCs.get(posCount));
            for(int dofIndex=0; dofIndex<rc.DOFs.size(); dofIndex++){
                if(rc.DOFs.get(dofIndex).getName().equalsIgnoreCase(dofName)){//found bounds on our dof
                    return new double[] {rc.DOFmin.get(dofIndex), rc.DOFmax.get(dofIndex)};
                }
            }
        }
        
        if(errorIfUndefined)//DOF not relevant to voxel
            throw new RuntimeException("ERROR: DOF "+dofName+" not defined for voxel: "+vox.stringListing());
        else
