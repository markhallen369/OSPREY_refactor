 ...
 * @author mhall44
 */
public class SparseLinearEnergy implements EnergyModel {
    
    FeatureSet featSet;
    double[] coeffs;
    
    public SparseLinearEnergy(FeatureSet featSet, EnergiedConfSampleSet trainingSet, double initCoeffs[]){
    ...
    public SparseLinearEnergy(EPICMatrix epicMat, PruningMatrix pruneMat, PolytopeMatrix plugMat){
        ...
        BasicConfSampleSet.VoxDrawTupleExpander te = new BasicConfSampleSet.VoxDrawTupleExpander(pruneMat,plugMat,featSet);
    ...
    private SLEFitter makeFitter(FeatureSet featSet, EnergiedConfSampleSet trainingSet){
        if(trainingSet instanceof BasicConfSampleSet)
            return new SLEFitter.BasicCGFitter(featSet,(BasicConfSampleSet)trainingSet);
        else if(trainingSet instanceof ContBatchSampleSet){
            if(featSet.freezeSAPE)
                return new SLEFitter.FrozenSAPECGFitter(featSet,(ContBatchSampleSet)trainingSet);
            else
                return new SLEFitter.ContBatchCGFitter(featSet,(ContBatchSampleSet)trainingSet);
        }
        else
            throw new RuntimeException("ERROR: Training set type not yet supported");
    }
    
    
    @Override
    public EnergyModel integrateModel(IntegrableDOF dof, EnergiedConfSampleSet samples){
        FeatureSet newFeat = dof.featureSetForInteg(featSet);
        double[] initCoeffs = initCoeffsForNewFeat(newFeat);
        EnergyModel newF = new SparseLinearEnergy(newFeat, samples, initCoeffs);
        return newF;
    }
