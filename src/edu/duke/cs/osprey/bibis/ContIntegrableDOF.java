...
import static edu.duke.cs.osprey.bibis.IntegrableDOF.RT;
...
public class ContIntegrableDOF implements IntegrableDOF {
    ...
    @Override
    public double boltzmannIntegratedEnergy(EnergyModel baseE, ConfSample samp) {
        ...
        DofPLUGBounds dpb = nonClashingBounds(samp, fixedDOFs);
        switch(dpb.status){
            case CLASHING:
            ...
            case BOUNDED:
                ...
                for(int step=0; step<=numIntegBins; step++){
                    ...
                    ConfSample altSamp = new ConfSample(samp.vox, altDOFVals, altDOFValsArr);
                    int wt = simpsonCoeff(step,numIntegBins);
                    q += wt * Math.exp(-baseE.evalEnergy(altSamp,mms)/RT);
                    wtSum += wt;
                }
    ...
    DofPLUGBounds nonClashingBounds(ConfSample samp, Iterable<DegreeOfFreedom> fixedDOFs){
        ...
        for(DegreeOfFreedom fixedDOF : fixedDOFs){
            ...
            q[allDOFNames.indexOf(fixedDOF.getName())] = 1;
            LinearConstraint eqConstr = new LinearConstraint( q, Relationship.EQ, samp.contDOFVals.get(fixedDOF.getName()) );
